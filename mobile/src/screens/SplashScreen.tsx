...
function TabScreen({ route, navigation }: any) {
  const screenConfigs = {
    HomeTab: {
      component: HomeScreen,
      title: "SmartHealth+",
  ...
  const config = screenConfigs[route.name as keyof typeof screenConfigs] || screenConfigs.HomeTab;
...
export default function AppNavigator() {
  ...
  const checkRegistrationStatus = async () => {
    try {
      ...
      const registrationStatus = await AsyncStorage.getItem('hasCompletedRegistration');
      const userProfile = await AsyncStorage.getItem('userProfile');

      if (registrationStatus === 'true' && userProfile) {
  ...
  if (isLoading) {
    return (
      <View style={styles.loadingContainer}>
        ...
        <Text style={styles.loadingText}>Loading SmartHealth+...</Text>
      </View>
    );
  }
  ...
  return (
    <NavigationContainer>
      ...
      >
        {/* Show Registration Screen first if not completed */}
        ...
        {/* Main Tab Navigator - Always show after registration */}
        <Stack.Screen
          name="MainTabs"
          component={MainTabs}
